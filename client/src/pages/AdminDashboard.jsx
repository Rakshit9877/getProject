...
export default function AdminDashboard() {
    ...
    const [couponForm, setCouponForm] = useState({
        code: '', discountType: 'percentage', discountValue: '', setsFixedPrice: false, fixedPrice: '', maxUses: '', expiresAt: '',
    })
    ...
    const fetchOrders = async () => {
        ...
        try {
            ...
            const res = await axios.get(`${API}/api/admin/orders`, { headers: { Authorization: `Bearer ${token}` }, params })
            setOrders(res.data)
        } catch (err) {
    ...
    const fetchCoupons = async () => {
        ...
        try {
            const res = await axios.get(`${API}/api/admin/coupons`, { headers: { Authorization: `Bearer ${token}` } })
            setCoupons(res.data)
    ...
    const updateStatus = async () => {
        ...
        setUpdating(true); setStatusSuccess(false)
        try {
    ...
    const createCoupon = async () => {
        try {
            ...
            await axios.post(`${API}/api/admin/coupons`, {
                ...
                discountType: isFixed ? 'flat' : couponForm.discountType,
                discountValue: isFixed ? 0 : Number(couponForm.discountValue),
                setsFixedPrice: isFixed,
                fixedPrice: isFixed ? Number(couponForm.fixedPrice) : null,
                maxUses: couponForm.maxUses ? Number(couponForm.maxUses) : null,
                expiresAt: couponForm.expiresAt || null,
            }, { headers: { Authorization: `Bearer ${token}` } })
            setCouponForm({ code: '', discountType: 'percentage', discountValue: '', setsFixedPrice: false, fixedPrice: '', maxUses: '', expiresAt: '' })
    ...
    const toggleCoupon = async (id, isActive) => {
        try {
            await axios.patch(`${API}/api/admin/coupons/${id}`, { isActive: !isActive }, { headers: { Authorization: `Bearer ${token}` } })
            fetchCoupons()
                    ...
                    <>
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                            {[
                                ...
                                { label: 'Pending', value: orders.filter(o => o.status === 'pending_verification').length, color: 'text-blue-400' },
                        ...
                        <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-2">
                            {['', ...allStatuses].map(status => (
                                <button key={status} onClick={() => setFilter(status)} className={`text-sm px-4 py-2 rounded-lg whitespace-nowrap transition-all ${filter === status ? 'bg-navy-500 text-white' : 'bg-navy-900/50 text-navy-400 hover:text-white border border-white/5'}`}>
                        ...
                        {loading ? (
                            <div className="text-center py-20"><p className="text-navy-400">Loading orders...</p></div>
                        ) : orders.length === 0 ? (
                            <div className="text-center py-20"><p className="text-navy-400 text-lg">No orders found</p></div>
                        ) : (
                                    ...
                                    <table className="w-full text-sm">
                                        <thead>
                                            <tr className="border-b border-white/10">
                                                ...
                                                <th className="text-left py-3 px-4 text-navy-400 font-medium text-xs uppercase tracking-wider">Order ID</th>
                                                <th className="text-left py-3 px-4 text-navy-400 font-medium text-xs uppercase tracking-wider">Name</th>
                                                <th className="text-left py-3 px-4 text-navy-400 font-medium text-xs uppercase tracking-wider hidden md:table-cell">Project</th>
                                        ...
                                        <tbody>
                                            {orders.map(order => (
                                                <tr key={order._id} onClick={() => openOrderDetail(order)} className="border-b border-white/5 hover:bg-white/5 cursor-pointer transition-colors">
                                                    <td className="py-3 px-4 text-navy-300 whitespace-nowrap">{formatDate(order.createdAt)}</td>
                                                    <td className="py-3 px-4 text-navy-400 font-mono text-xs">{order.orderId?.substring(0, 16) || '—'}...</td>
                ...
                {activeTab === 'coupons' && (
                    <>
                        <div className="flex items-center justify-between mb-4">
                            ...
                            <button onClick={() => setShowCouponForm(!showCouponForm)} className="btn-primary text-sm">{showCouponForm ? 'Cancel' : '+ Create Coupon'}</button>
                        </div>
                        <p className="text-navy-500 text-xs mb-6">You can also seed coupons by running: <code className="bg-navy-900/50 px-2 py-0.5 rounded text-navy-400">node server/scripts/seedCoupons.js</code></p>
                            ...
                            <div className="glass-card p-6 mb-6 space-y-4">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <input type="text" placeholder="e.g. LAUNCH50" value={couponForm.code} onChange={e => setCouponForm(p => ({ ...p, code: e.target.value }))} className="input-field font-mono uppercase text-sm" />
                                    </div>
                                    <div>
                                    ...
                                    {couponForm.discountType === 'fixed_price' ? (
                                        <div>
                                            <label className="text-xs text-navy-500 block mb-1">Fixed Price (₹)</label>
                                            <input type="number" placeholder="e.g. 1" value={couponForm.fixedPrice} onChange={e => setCouponForm(p => ({ ...p, fixedPrice: e.target.value }))} className="input-field text-sm" />
                                        </div>
                                    ) : (
                                        <div>
                                            <label className="text-xs text-navy-500 block mb-1">Discount Value {couponForm.discountType === 'percentage' ? '(%)' : '(₹)'}</label>
                                            <input type="number" placeholder={couponForm.discountType === 'percentage' ? 'e.g. 50' : 'e.g. 200'} value={couponForm.discountValue} onChange={e => setCouponForm(p => ({ ...p, discountValue: e.target.value }))} className="input-field text-sm" />
                                        </div>
                                    )}
                                    <div>
                                        <label className="text-xs text-navy-500 block mb-1">Max Uses (blank = unlimited)</label>
                                        <input type="number" placeholder="Unlimited" value={couponForm.maxUses} onChange={e => setCouponForm(p => ({ ...p, maxUses: e.target.value }))} className="input-field text-sm" />
                                    </div>
                                    <div>
                                        <label className="text-xs text-navy-500 block mb-1">Expires At (optional)</label>
                                        <input type="date" value={couponForm.expiresAt} onChange={e => setCouponForm(p => ({ ...p, expiresAt: e.target.value }))} className="input-field text-sm" />
                                    </div>
                                </div>
                        ...
                        {couponsLoading ? (
                            <div className="text-center py-20"><p className="text-navy-400">Loading coupons...</p></div>
                        ) : coupons.length === 0 ? (
                            <div className="text-center py-20"><p className="text-navy-400 text-lg">No coupons yet</p></div>
                        ) : (
                            <div className="glass-card overflow-hidden">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        ...
                                        <tbody>
                                            {coupons.map(c => (
                                                <tr key={c._id} className="border-b border-white/5">
                                                    ...
                                                    <td className="py-3 px-4 text-navy-300 capitalize">{c.setsFixedPrice ? 'Fixed Price' : c.discountType}</td>
                                                    <td className="py-3 px-4 text-white">{c.setsFixedPrice ? `₹${c.fixedPrice}` : c.discountType === 'percentage' ? `${c.discountValue}%` : `₹${c.discountValue}`}</td>
                                                    ...
                                                    <td className="py-3 px-4">
                                                        <button onClick={() => toggleCoupon(c._id, c.isActive)} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${c.isActive ? 'bg-emerald-500' : 'bg-navy-700'}`}>
                        ...
                        <div className="p-5 space-y-5">
                            ...
                            <div className="bg-navy-800/50 rounded-xl p-4 space-y-3">
                                ...
                                <div>
                                    ...
                                    <textarea value={statusForm.message} onChange={(e) => setStatusForm(p => ({ ...p, message: e.target.value.slice(0, 300) }))} placeholder="e.g. We've completed the authentication module..." rows={3} className="input-field text-sm w-full resize-none" />
                                </div>
                                <div className="flex items-center gap-3">
                                    <button onClick={updateStatus} disabled={updating} className="btn-primary text-sm !py-2">{updating ? 'Sending...' : 'Send Update'}</button>
                                    {statusSuccess && <span className="text-emerald-400 text-sm flex items-center gap-1">✓ Status updated!</span>}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3 text-sm">
                                    ...
                                    {selectedOrder.selectedFeatures?.length > 0 && (
                                        <div>
                                            <span className="text-navy-500 block mb-1">Selected Features</span>
                                            <div className="flex flex-wrap gap-1.5">{selectedOrder.selectedFeatures.map(f => <span key={f} className="bg-navy-500/20 text-navy-300 text-xs px-2 py-0.5 rounded-lg">{f}</span>)}</div>
                                        </div>
                                    )}
                                    {selectedOrder.techStack?.length > 0 && (
                                        <div>
                                            <span className="text-navy-500 block mb-1">Auto-derived Tech Stack</span>
                                            <div className="flex flex-wrap gap-1.5">{selectedOrder.techStack.map(t => <span key={t} className="bg-navy-600/20 text-navy-400 text-xs px-2 py-0.5 rounded-lg">{t}</span>)}</div>
                                        </div>
                                    )}
                                    ...
                                    {selectedOrder.featureList && <div><span className="text-navy-500 block">Additional Notes</span><span className="text-white whitespace-pre-line">{selectedOrder.featureList}</span></div>}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="space-y-3 text-sm">
                                    <div>
                                        ...
                                        <a href={selectedOrder.githubRepoUrl} target="_blank" rel="noopener noreferrer" className="text-navy-400 hover:text-navy-300 font-mono text-xs break-all">{selectedOrder.githubRepoUrl}</a>
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                        <div><span className="text-navy-500 block">Order ID</span><span className="text-white font-mono text-xs">{selectedOrder.orderId}</span></div>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <div>
                                            <span className="text-navy-500 block">Amount Paid</span>
                                            <span className="text-white font-semibold">₹{selectedOrder.finalAmountPaid?.toLocaleString('en-IN') || '—'}</span>
