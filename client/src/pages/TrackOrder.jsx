          ...
          <div className="glass-card p-6 sm:p-8 space-y-6 animate-fade-in">
            ...
            {order.status === 'refunded' ? (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-5 text-center">
                ...
                <p className="text-red-400/70 text-sm mt-1">This order has been refunded.</p>
              </div>
            ) : (
                    ...
                    {stages.map((stage, i) => (
                      <div key={stage.key} className="relative flex flex-col items-center z-10" style={{ width: `${100 / stages.length}%` }}>
                        ...
                        <span className={`text-xs mt-2 text-center leading-tight ${
                          i <= activeIndex ? 'text-white font-medium' : 'text-navy-600'
                        }`}>{stage.label}</span>
                      </div>
                    ))}
                  </div>
                    ...
                    <div key={stage.key} className="flex items-start gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs border-2 ${
                          ...
                          {i < activeIndex ? '✓' : stage.icon}
                        </div>
                        {i < stages.length - 1 && (
                          <div className={`w-0.5 h-6 ${i < activeIndex ? 'bg-emerald-500' : 'bg-navy-800'}`} />
                        )}
                      </div>
            ...
            {order.statusMessage && (
              <div className="bg-navy-500/5 border border-navy-500/20 rounded-xl p-4">
                ...
                {order.statusUpdatedAt && (
                  <p className="text-xs text-navy-500 mt-2">{new Date(order.statusUpdatedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                )}
              </div>
            )}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              ...
              <div className="sm:col-span-2 bg-navy-900/50 border border-white/10 rounded-xl p-4">
                ...
                <a href={order.githubRepoUrl} target="_blank" rel="noopener noreferrer" className="text-navy-400 hover:text-navy-300 font-mono text-xs break-all">
                  {order.githubRepoUrl}
                </a>
              </div>
            </div>
