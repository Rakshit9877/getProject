...
export default function OrderForm() {
  ...
  const validateStep = (step) => {
    ...
    if (step === 1) {
      ...
      if (!formData.featureCount) newErrors.featureCount = 'Feature count is required'
      if (formData.featureList.length > 300) newErrors.featureList = 'Feature list must be 300 characters or less'
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const initiateRes = await axios.post(`${API}/api/orders/initiate`, {
        ...
        featureCount: formData.featureCount,
