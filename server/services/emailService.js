...
async function sendCustomerConfirmation(order) {
    const mailOptions = {
        from: `"ProjexLab" <${process.env.EMAIL_USER}>`,
        to: order.email,
      ...
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          ...
          <div style="background: #eff6ff; border-left: 4px solid #6366f1; padding: 16px; border-radius: 4px; margin: 20px 0;">
            <p style="color: #334155; margin: 0;"><strong>Track your order:</strong></p>
            <p style="color: #475569; margin: 8px 0 0 0;">Use your Order ID to track progress anytime: <code style="background: #e2e8f0; padding: 2px 6px; border-radius: 4px;">${order._id}</code></p>
          </div>
    ...
    const mailOptions = {
      ...
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        <table style="width: 100%; border-collapse: collapse;">
          ...
          <tr><td style="padding: 8px; color: #64748b;">Amount</td><td style="padding: 8px;">₹${order.amount}${order.discountApplied ? ` (₹${order.discountApplied} discount via ${order.couponCode})` : ''}</td></tr>
        </table>
...
async function sendStatusUpdate(order) {
    const statusMessages = {
        pending_verification: {
            ...
            body: 'Your order has been received. We are verifying your GitHub collaborator access.',
        },
        collaborator_verified: {
            emoji: '✅',
            title: 'Collaborator Verified',
            body: 'GitHub collaborator access has been confirmed. Your project is queued for development.',
        },
        in_progress: {
            emoji: '🚀',
        ...
        review_testing: {
            ...
            body: 'Your project is in the review and testing phase. We are ensuring code quality before delivery.',
        },
        completed: {
            emoji: '🎉',
            title: 'Project Completed!',
            body: 'Your project has been completed and all code has been pushed to your GitHub repository.',
        },
        refunded: {
            emoji: '💸',
            title: 'Refund Processed',
            body: 'Your order has been refunded. The amount will be credited back within 5–7 business days.',
        },
    };
    ...
    const mailOptions = {
      ...
      <div style="font-family: 'Segoe UI', sans-serif; max-width: 600px; margin: 0 auto; background: #f8fafc; border-radius: 12px; overflow: hidden;">
        ...
        <div style="padding: 32px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <tr><td style="padding: 8px 0; color: #64748b; border-bottom: 1px solid #e2e8f0;">Project</td><td style="padding: 8px 0; color: #0f172a; border-bottom: 1px solid #e2e8f0; font-weight: 600;">${order.projectTitle}</td></tr>
            <tr><td style="padding: 8px 0; color: #64748b; border-bottom: 1px solid #e2e8f0;">Status</td><td style="padding: 8px 0; color: #6366f1; border-bottom: 1px solid #e2e8f0; font-weight: 600; text-transform: uppercase;">${order.status.replace(/_/g, ' ')}</td></tr>
            <tr><td style="padding: 8px 0; color: #64748b;">GitHub Repo</td><td style="padding: 8px 0; color: #0f172a; font-weight: 600;">${order.githubRepoUrl}</td></tr>
          </table>
          <p style="color: #64748b; font-size: 14px;">Questions? Contact us at <a href="mailto:${process.env.ADMIN_EMAIL}" style="color: #6366f1;">${process.env.ADMIN_EMAIL}</a></p>
        </div>
